...
main(int argc, char *argv[])
{
	...
	{
		FILE *hackhack;
		hackhack = fopen("autominfo.txt", "w");
	    while (TRUE)
	    {
		...
		loops = loopcount(g,m,n);
			strcpy(autominfo, "");
	        //for (ii = 0; ii < secret; ++ii)
	            fcanonise_inv2(g,m,n,h,fmt,invarproc[inv].entrypoint,
			        mininvarlevel,maxinvarlevel,invararg,loops>0, autominfo);
	        //fprintf(stdout, "%lu : %s\n", nin, autominfo);
	        fprintf(hackhack, "%lu : %s\n", nin, autominfo);
	        orbtotal += gt_numorbits;
	        unorbtotal += 1.0 / gt_numorbits;
